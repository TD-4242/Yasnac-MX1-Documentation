...
properties = {
  ...
  dOffset: 0, // specifies a value to add to radius compensation register
  usePitchForTapping: true, // enable to use pitch output for tapping
  useSolidTapping: false, // enables solid tapping (G93) mode
...
var gFormat = createFormat({prefix:"G", width:0, zeropad:false, decimals:0});
var mFormat = createFormat({prefix:"M", width:0, zeropad:false, decimals:0});
var hFormat = createFormat({prefix:"H", width:0, zeropad:false, decimals:0});
var dFormat = createFormat({prefix:"D", width:0, zeropad:false, decimals:0});

var xyzFormat = createFormat({decimals:(unit == MM ? 3 : 4), forceDecimal:true});
var abcFormat = createFormat({decimals:3, forceDecimal:true, scale:DEG});
var feedFormat = createFormat({decimals:(unit == MM ? 0 : 1), forceDecimal:true});
var toolFormat = createFormat({width:0, zeropad:false, decimals:0}); // change to 2 for old controls
...
function onOpen() {
  ...
  if (programName) {
    ...
    var oFormat;
    if (properties.o5) {
      oFormat = createFormat({width:5, zeropad:false, decimals:0});
      maxSeqno = 99999;
    } else {
      oFormat = createFormat({width:4, zeropad:false, decimals:0});
      maxSeqno = 9999;
    }
